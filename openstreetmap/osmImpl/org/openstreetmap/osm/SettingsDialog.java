...
/**
 * This dialog takes all the settings in {@link Settings} and shows a nice dialog to edit them. It
 * is fully self-contained and needs only to be shown.
 * 
...
public class SettingsDialog extends JFrame
{
	...
	/**
	 * This is a special Property-class that adapts a {@link ConfigurationSetting} into a Property.
	 * 
	 * @author <a href="mailto:Marcus@Wolschon.biz">Marcus Wolschon</a>
	 */
	private static class MyProperty extends DefaultProperty
	{
		...
		public MyProperty(final ConfigurationSetting setting)
		{
			...
			{
				case BOOLEAN:
					setValue(settings.getBoolean(setting.getKey(), false));
					break;
				case DOUBLE:
					setValue(settings.getDouble(setting.getKey(), 0.0));
					break;
				case INTEGER:
					setValue(settings.getInteger(setting.getKey(), 0));
					break;
				default:
					setValue(settings.get(setting.getKey(), null));
			}
	...
	/**
	 * We extend the PropertyEditorRegistry to create a {@link PluginPropertyEditor} if needed.
	 * 
	 * @author <a href="mailto:Marcus@Wolschon.biz">Marcus Wolschon</a>
	 */
	private static class MyPropertyEditorFactory extends PropertyEditorRegistry implements
			PropertyEditorFactory, PropertyRendererFactory
	{

		/**
		 * Default for #createTableCellRenderer.
		 */
		private PropertyRendererRegistry myPropertyRendererRegistry =
		...
		{
			return this.myPropertyRendererRegistry.createTableCellRenderer(aClass);
		}
		...
		public TableCellRenderer createTableCellRenderer(final Property aProperty)
		{
			...
			return this.myPropertyRendererRegistry.createTableCellRenderer(aProperty);
		}
	}
	 ...
	 * @author <a href="mailto:Marcus@Wolschon.biz">Marcus Wolschon</a>
	 */
	private static class PluginPropertyEditor extends AbstractPropertyEditor implements
			PropertyEditor, PropertyChangeListener, TableCellRenderer
	{
		...
		public PluginPropertyEditor(final ConfigurationSetting aConfigurationSetting)
		{
			...
			Map<String, String> registredPlugins =
					Settings.getRegistredPlugins(aConfigurationSetting.getPluginInterface());
			...
			String value = Settings.getInstance().get(this.myConfigurationSetting.getKey(), "");
			for (int i = 0; i < this.myComboBox.getItemCount(); i++)
			{
				PluginValue comboValue = (PluginValue) this.myComboBox.getItemAt(i);
				...
				public void itemStateChanged(final ItemEvent aE)
				{
					if (aE.getStateChange() == ItemEvent.SELECTED)
					{
						...
						if (!Settings.getInstance()
								.get(PluginPropertyEditor.this.myConfigurationSetting.getKey(), "")
								.equals(value.getValue()))
						{
							firePropertyChange(
									Settings.getInstance().get(
											PluginPropertyEditor.this.myConfigurationSetting
													.getKey(), null), value.getValue());
							Settings.getInstance().put(
									PluginPropertyEditor.this.myConfigurationSetting.getKey(),
									value.getValue());
						}
					}
				}
		...
		public Component getCustomEditor()
		{
			...
			String val = Settings.getInstance().get(this.myConfigurationSetting.getKey(), null);
			int index = 0;
			Map<String, String> registredPlugins =
					Settings.getRegistredPlugins(this.myConfigurationSetting.getPluginInterface());
			...
			else
			{
				if (this.myComboBox.getItemCount() > 0)
				{
					...
					Settings.getInstance().put(this.myConfigurationSetting.getKey(),
							((PluginValue) this.myComboBox.getSelectedItem()).getValue());
				}
			}
			customEditor.add(getSelectedPluginSettings(), BorderLayout.EAST);
			if (this.myPluginSettings == null)
			{
		...
		{
			if (this.mySelectedPluginSettings == null)
			{
				this.mySelectedPluginSettings = new JButton("...");
				this.mySelectedPluginSettings.addActionListener(new ActionListener()
				{
					...
					public void actionPerformed(final ActionEvent aE)
					{
						...
						IPlugin plugin =
								Settings.getInstance().getPlugin(
										PluginPropertyEditor.this.myConfigurationSetting
												.getPluginInterface(), null);
						SettingsDialog pluginSetting =
								new SettingsDialog(PluginPropertyEditor.this.myPluginSettings);
						pluginSetting.setTitle(pluginSetting.getTitle() + " - " + value);
						pluginSetting.setVisible(true);
					}
				});
			}
			return this.mySelectedPluginSettings;
		}
		...
		// return
		// Settings.RESOURCE.getString("libosm.settingsdialog.pluginPropertyEditor.hasSettingsPrefix")
		// + super.getAsText();
		// }
		...
		public Component getTableCellRendererComponent(final JTable aTable, final Object aValue,
														...
														final boolean aHasFocus, final int aRow,
														final int aColumn)
		{
			String value = Settings.getInstance().get(this.myConfigurationSetting.getKey(), "");
			if (value.startsWith("JPF:"))
			...
			this.mySelectPluginLabel = new JLabel(value);

			JPanel customEditor = new JPanel(new BorderLayout());
			customEditor.add(this.mySelectPluginLabel, BorderLayout.CENTER);
			customEditor.add(getSelectedPluginSettings(), BorderLayout.EAST);
			if (this.myPluginSettings == null)
			{
				getSelectedPluginSettings().setEnabled(false);
		...
		public void propertyChange(final PropertyChangeEvent aEvt)
		{
			IPlugin plugin =
					Settings.getInstance().getPlugin(
							this.myConfigurationSetting.getPluginInterface(), null);
			...
			// update the label
			if (this.mySelectPluginLabel != null)
			{
				if (value.startsWith("JPF:"))
				{
					value = value.substring("JPF:".length());
				...
				this.mySelectPluginLabel.setText(value);
			}
		}
	}
	...
	/**
	 * A panel to show a config-section and it's sub-sections.
	 * 
	 * @author <a href="mailto:Marcus@Wolschon.biz">Marcus Wolschon</a>
	 */
	private static class PreferencesPanel extends JPanel
	{
		...
		/**
		 * This PropertyChangeListener checks if the property is a Setting and saves it.
		 * 
		 * @author <a href="mailto:Marcus@Wolschon.biz">Marcus Wolschon</a>
		 */
		private static final class SavingPropertyChangeListener implements PropertyChangeListener
		{
			...
			public void propertyChange(final PropertyChangeEvent evt)
			{
				...
				if (property instanceof MyProperty)
				{
					...
					{
						Settings.getInstance().put(prop.getSetting().getKey(),
								evt.getNewValue().toString());
					}
					if (prop.getSetting().isRequiredRestart())
					{
						javax.swing.JOptionPane.showMessageDialog(null, Settings.RESOURCE
								.getString("libosm.settingsdialog.message.settingRequiresRestart"));
					}
				}
			}
		}
		...
		public PreferencesPanel(final ConfigurationSection aConfigSection)
		{
			...
			this.myConfigSection = aConfigSection;
			...
			if (getConfigSection().getSubSections().size() > 0)
			{
				...
				{
					if ((subSection.getSettings().size() > 0)
		...
		private JPanel buildMySettingsPanel()
		{
			...
			propertySheet.setMode(PropertySheet.VIEW_AS_CATEGORIES);
			propertySheet.setDescriptionVisible(true);
			SavingPropertyChangeListener savingPropertyChangeListener =
					new SavingPropertyChangeListener();
	...
	/**
	 * This is a special PropertyEditor for the Settings that have a limited number of choices to
	 * select from.
	 * 
	 * @author <a href="mailto:Marcus@Wolschon.biz">Marcus Wolschon</a>
	 */
	private static class SelectPropertyEditor extends ComboBoxPropertyEditor implements
			PropertyEditor
	{
	...
	private JButton closeButton = new JButton(
			Settings.RESOURCE.getString("libosm.settingsdialog.button.close"));
	...
	/**
	 * Create the dialog. It is automaticall populated from
	 * {@link Settings#getRootConfigurationSection()}
	 */
	...
	public SettingsDialog(final ConfigurationSection config)
	{
		...
		buttonsPanel.add(this.closeButton, BorderLayout.EAST);
		...
		this.closeButton.addActionListener(new ActionListener()
		{
			...
			public void actionPerformed(final ActionEvent arg0)
			{
				...
				{
					settings.put("state.SettingsDialog.width",
							String.valueOf(SettingsDialog.this.getWidth()));
					settings.put("state.SettingsDialog.height",
							String.valueOf(SettingsDialog.this.getHeight()));
					settings.put("state.SettingsDialog.x",
							String.valueOf(SettingsDialog.this.getX()));
					settings.put("state.SettingsDialog.y",
							String.valueOf(SettingsDialog.this.getY()));
				}
				SettingsDialog.this.dispose();
			}
		});

		setTitle(Settings.RESOURCE.getString("libosm.settingsdialog.title"));
		setLayout(new BorderLayout());
		...
		if (settings.getBoolean("traveling-salesman.restoreLastState", true))
		{
			...
			height =
					settings.getInteger("state.SettingsDialog.height", DEFAULTSETTINGSDIALOGHEIGHT);
