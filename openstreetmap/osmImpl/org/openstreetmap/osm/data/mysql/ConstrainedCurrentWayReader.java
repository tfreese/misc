 ...
 */
public class ConstrainedCurrentWayReader extends ConstrainedBaseReader<Way> {

    /**
     * my logger for debug and error-output.
     */
    static final Logger LOG = Logger.getLogger(ConstrainedCurrentWayReader.class.getName());
        ...
        private static final String SELECT_BOUNDINGBOX_SQL =
            ...
            + " w.timestamp as way_timestamp, "
            + " u.display_name"
            + " FROM current_ways w, current_way_nodes wn, current_nodes n, users u {5}"
         ...
         */
        public ConstrainedCurrentWayReader(final DatabaseLoginCredentials aLoginCredentials) {
         ...
         */
        public ConstrainedCurrentWayReader(final DatabaseLoginCredentials aLoginCredentials, final long aNodeID) {
            this(aLoginCredentials);
        ...
        public void reInitializeForNode(final long aNodeID) {
            additionalFrom = ", current_way_nodes wn2";
            additionalWhere = " AND wn2.id = w.id AND wn2.node_id = " + aNodeID + " ";
        ...
        public ConstrainedCurrentWayReader(final DatabaseLoginCredentials aLoginCredentials,
                ...
                final double aminLat, final double aminLon, final double amaxLat, final double amaxLon) {
            this(aLoginCredentials, aTags, aOr, aLike);
            this.minLat = FixedPrecisionCoordinateConvertor.convertToFixed(Math.min(aminLat, amaxLat));
            this.maxLat = FixedPrecisionCoordinateConvertor.convertToFixed(Math.max(aminLat, amaxLat));
            this.minLon = FixedPrecisionCoordinateConvertor.convertToFixed(Math.min(aminLon, amaxLon));
            this.maxLon = FixedPrecisionCoordinateConvertor.convertToFixed(Math.max(aminLon, amaxLon));
        ...
        public Way getNext() {
            ...
            way = new Way(wayId, version, timestamp, null, tags, wayNodes);
            return way;
        }
    };
