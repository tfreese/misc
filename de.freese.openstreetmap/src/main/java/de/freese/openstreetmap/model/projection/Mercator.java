...
/**
 * Implement Mercator Projection code, coded after documentation from wikipedia. The center of the
 * mercator projection is always the 0 grad coordinate.
 * 
 * @author imi
...
public class Mercator implements Projection
{
	...
	{
		return new LatLon((Math.atan(Math.sinh(p.north())) * C_180) / Math.PI, (p.east() * C_180)
				/ Math.PI);
	}
	...
	{
		return new EastNorth((lon * Math.PI) / C_180, Math.log(Math.tan(QUARTERPI
				+ ((lat * Math.PI) / C_360))));
	}
