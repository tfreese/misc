...
/**
 * Provides utility methods to dump an {@link AbstractIoBuffer} into a hex formatted string.
 *
...
class IoBufferHexDumper
{
    ...
    /**
     * Dumps an {@link AbstractIoBuffer} to a hex formatted string.
     *
     * @param in the buffer to dump
     * @param lengthLimit the limit at which hex dumping will stop
     * @return a hex formatted string representation of the <i>in</i> {@link AbstractIoBuffer}.
     */
    public static String getHexdump(final AbstractIoBuffer in, final int lengthLimit)
    {
        if (lengthLimit == 0)
        {
