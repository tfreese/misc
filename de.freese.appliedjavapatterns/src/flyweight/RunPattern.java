...
public class RunPattern
{
	 ...
	 * @throws java.io.IOException Falls was schief geht
	 */
	public static void main(final String[] arguments) throws java.io.IOException
	{
		...
		StateFactory.getCurrentState().save(new File("contacts.ser"),
				new ArrayList<>(contactList.getItems()), State.CONTACTS);
		System.out.println("Printing the State for the application");
		printPIMState();
		System.out.println();

		System.out.println("Saving the Address list");
		StateFactory.getCurrentState().save(new File("addresses.ser"),
				new ArrayList<>(addressList.getItems()), State.ADDRESSES);
		System.out.println("Printing the State for the application");
		printPIMState();
	}
	...
	{
		System.out.println("  Current State of the PIM: "
				+ StateFactory.getCurrentState().getClass());
