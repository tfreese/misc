...
public class TestJulianDay
{
	...
	public void fristBerechnungMonat()
	{
		...
		Assert.assertEquals(2456117, jd);

		Day d2 = JulianDayConverter.createDayFromJD(jd);
		Assert.assertEquals(new Day(2012, 7, 8), d2);
		Assert.assertEquals("2012-07-08", d2.toString());
	}
	...
	public void fristBerechnungTag()
	{
		...
		int jd = JulianDayConverter.calculateJD(d1);
		Assert.assertEquals(2456107, jd);

		Day d2 = JulianDayConverter.createDayFromJD(jd);
		Assert.assertEquals(new Day(2012, 6, 28), d2);
		Assert.assertEquals("2012-06-28", d2.toString());
	}
	...
	public void konvertierungJavaUtilDate()
	{
		...
		Assert.assertEquals("2012-08-14", JulianDayConverter.createDayFromJD(jdBerlin).toString());
		...
		Assert.assertEquals("2012-08-13", JulianDayConverter.createDayFromJD(jdLosAngeles).toString());
	}
	...
	public void vergleich()
	{
		...
		int jd = JulianDayConverter.calculateJD(d1);
		Assert.assertEquals(2447893, jd);

		Day d2 = JulianDayConverter.createDayFromJD(jd);
		Assert.assertEquals(d1, d2);
		Assert.assertEquals(d1.toString(), d2.toString());
		Assert.assertEquals("1990-01-01", d2.toString());
	}
	...
	public void wochentagsErmittlung()
	{
		...
		int jd = JulianDayConverter.calculateJD(d1);
		Assert.assertEquals(0, jd % 7); // Montag == 0

		Day d2 = JulianDayConverter.createDayFromJD(jd);
		Assert.assertEquals(d1, d2);
		Assert.assertEquals(d1.toString(), d2.toString());
		Assert.assertEquals("2012-08-06", d2.toString());
	}
}
