...
public class HttpRequestDecoder extends MessageDecoderAdapter
{
	...
	/**
	 * Erstellt ein neues {@link HttpRequestDecoder} Object.
	 */
	...
	/**
	 * @see org.apache.mina.filter.codec.demux.MessageDecoder#decodable(org.apache.mina.core.session.IoSession,
	...
	/**
	 * @see org.apache.mina.filter.codec.demux.MessageDecoder#decode(org.apache.mina.core.session.IoSession,
	 *      org.apache.mina.core.buffer.IoBuffer,
	 ...
	 */
	private boolean messageComplete(final IoBuffer in) throws Exception
	{
			...
			for (int i = 0; i < last; i++)
			{
				...
				if (found)
				{
					...
					for (int j = i + CONTENT_LENGTH.length; j < last; j++)
					{
						...
						contentLength.append(new String(new byte[]
						{
							in.get(j)
						}));
					}
	 ...
	 */
	private HttpRequestMessage decodeBody(final IoBuffer in)
	{
		this.request = new HttpRequestMessage();
		try
		{
			this.request.setHeaders(parseRequest(new StringReader(in.getString(this.decoder))));
			return this.request;
		}
	 ...
	 * @return {@link Map}
	 */
	private Map<String, String[]> parseRequest(final Reader is)
	{
