...
public class PortScanner
{
	...
	public static void main(final String[] args)
	{
		...
		LOGGER.info(String.format("host: %s, ports: %d-%d, threads: %d", host,
				Integer.valueOf(firstPort), Integer.valueOf(lastPort), Integer.valueOf(threads)));

		final ExecutorService executor = Executors.newFixedThreadPool(threads);
		// new ThreadPoolExecutor(1, threads, 5L, TimeUnit.SECONDS,
		// new SynchronousQueue<Runnable>(), new ThreadPoolExecutor.CallerRunsPolicy());

		final Map<Integer, Port> openPorts =
				Collections.synchronizedMap(new TreeMap<Integer, Port>());
		...
		executor.execute(new Runnable()
		{
			...
			public void run()
			{
				LOGGER.info("ThreadPool (Active): {}",
						Integer.valueOf(((ThreadPoolExecutor) executor).getActiveCount()));
				LOGGER.info("ThreadPool (Tasks): {}",
						Long.valueOf(((ThreadPoolExecutor) executor).getTaskCount()));
				LOGGER.info("ThreadPool (Queue): {}",
						Integer.valueOf(((ThreadPoolExecutor) executor).getQueue().size()));
