...
/**
 * The <code>Sudoku</code> class povides a static <code>main</code> method allowing it to be called from the command line to print the solution to a specified
 * Sudoku problem.
 * <p>
 ...
 * </pre>
 *
 * Note that the first row <code>187492563</code> contains each number exactly once, as does the first column <code>159426873</code>, the upper-left box
 * <code>187534962</code>, and every other row, column and box.
 * <p>
 * The {@link #main(String[])} method encodes a problem as an array of strings, with one string encoding each constraint in the problem in row-column-value
 ...
 * </pre>
 *
 * The <code>8</code> in the upper left box of the puzzle is encoded as <code>018</code> ( <code>0</code> for the row, <code>1</code> for the column, and
 * <code>8</code> for the value). The <code>4</code> in the lower right box is encoded as <code>874</code>.
 * <p>
...
public class Sudoku
{
     ...
     * @return boolean
     */
    static boolean legal(final int i, final int j, final int val, final int[][] cells)
    {
        for (int k = 0; k < 9; ++k)
        {
            if (val == cells[k][j])
     ...
     * @return boolean
     */
    static boolean solve(int i, int j, final int[][] cells)
    {
        if (i == 9)
        {
            i = 0;
