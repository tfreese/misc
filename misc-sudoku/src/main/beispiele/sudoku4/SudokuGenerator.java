...
public class SudokuGenerator
{
     ...
     *
     * @param size size of the Sudoku grid is received as parameter actually not the size of the grid but the size of the region
     */
    ...
    /**
     * fill the ArrayList with all available number for that row,col returns the number of elements in the arraylist
     *
     * @param row int
     * @param col int
     * @return int
    ...
    /**
     * try to generate an return true if it worked (it might be impossible to fill the row for example if the only element left in the row is 5 so the last
     * column must be 5 but I may already have a 5 in the column or region)
     *
     * @param row int
     * @return boolean
     */
    ...
    public String toString()
    {
        ...
        StringBuilder buffer = new StringBuilder(this.size * this.size * this.size);
        buffer.append('+');
