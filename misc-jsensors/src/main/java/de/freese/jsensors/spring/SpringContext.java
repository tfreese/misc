...
public class SpringContext implements ApplicationContextAware, ResourceLoaderAware, EnvironmentAware, BeanFactoryAware, InitializingBean
{
    ...
    {
        BeanDefinitionRegistry bdr = (BeanDefinitionRegistry) getApplicationContext();
        bdr.registerBeanDefinition(beanID, beanDefinition);
    }
    ...
    {
        if (getApplicationContext() instanceof SingletonBeanRegistry)
        {
            SingletonBeanRegistry sbr = (SingletonBeanRegistry) getApplicationContext();
            sbr.registerSingleton(beanID, bean);
        }
        else if (getBeanFactory() instanceof SingletonBeanRegistry)
        {
            SingletonBeanRegistry sbr = (SingletonBeanRegistry) getBeanFactory();
            sbr.registerSingleton(beanID, bean);
        }
        else
        {
            throw new UnsupportedOperationException("SingletonBeanRegistry not found");
        }
    }
    ...
    {
        if (SpringContext.beanFactory != null)
        {
            throw new IllegalStateException("beanFactory already set !");
        }

        SpringContext.beanFactory = Objects.requireNonNull(beanFactory, "beanFactory required");
    }
