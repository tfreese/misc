...
public class GridDrawer
{
	...
	GridDrawer(final int w, final int h)
	{
		...
		this.squareWidth =
				Math.min((w - 2 * this.xOffset) / 9, (h - this.xOffset - this.yOffset) / 9);
	 ...
	 */
	public void paintSudoku(final Graphics2D g, final int[][] grid, final int[][] user,
							final int x, final int y, final int state, final boolean showValids,
							final int difficulty)
	{
		...
		if ((x >= 0) && (y >= 0))
		{
			...
			g.fillRect(this.xOffset + x * this.squareWidth, this.yOffset + y * this.squareHeight,
			...
			for (int j = 0; j < N; j++)
			{
				...
				if (state == Sudoku.SHOW_ERRORS)
				{
					if (user[i][j] != 0)
					{
						...
						g.fillRect(this.xOffset + i * this.squareWidth, this.yOffset + j
								* this.squareHeight, this.squareWidth, this.squareHeight);
					}
				...
				if (grid[i][j] != 0)
				{
					...
					{
						g.fillOval(this.xOffset + i * this.squareWidth + 1, this.yOffset + j
								* this.squareHeight + 1, this.squareWidth - 2,
								this.squareHeight - 2);
					}
					else
					{
					...
					g.setColor(GridDrawer.CLR_TEXT);
					g.drawString((Sudoku.OPTION_ALPHA ? String.valueOf((char) (64 + grid[i][j]))
							: "" + grid[i][j]), this.xOffset + i * this.squareWidth + txtXOffset,
							this.yOffset + j * this.squareHeight + txtYOffset);
				}
				else
				{
					if (user[i][j] != 0)
					{
						...
						g.setColor(GridDrawer.CLR_TEXT);
						g.drawString(
								(Sudoku.OPTION_ALPHA ? String.valueOf((char) (64 + user[i][j]))
										: "" + user[i][j]), this.xOffset + i * this.squareWidth
										+ txtXOffset, this.yOffset + j * this.squareHeight
										+ txtYOffset);
					}
				}
			}
					...
					if (user[i][j] == 0)
					{
						...
						for (int k = 0; k < N; k++)
						{
							...
							if (this.V.get(i, j, k))
							{
								...
								int a = (k < 3 ? 0 : (k < 6 ? 1 : 2));
								int b = k % 3;
								g.drawString("" + (k + 1), this.xOffset + i * this.squareWidth
										+ (11 * b + 2), this.yOffset + j * this.squareHeight
										+ (11 * a + 9));
							}
						}
					}
		...
		g.drawRect(this.xOffset - 3, this.yOffset - 3, this.squareWidth * N + 6, this.squareHeight
				* N + 6);
		for (int i = 0; i < n; i++)
		{
			for (int j = 0; j < n; j++)
			{
				g.drawRect(this.xOffset - 1 + this.squareWidth * n * i, this.yOffset - 1
						+ this.squareHeight * n * j, this.squareWidth * n + 2, this.squareHeight
						* n + 2);
			}
		}
		for (int i = 0; i <= N; i++)
		{
			g.drawLine(this.xOffset, this.yOffset + this.squareHeight * i, this.xOffset
					+ this.squareWidth * N, this.yOffset + this.squareHeight * i);
			g.drawLine(this.xOffset + this.squareWidth * i, this.yOffset, this.xOffset
					+ this.squareWidth * i, this.yOffset + this.squareHeight * N);
		}
		...
		// Copyright
		g.setFont(GridDrawer.FNT_SMALL);
		g.drawString("Copyright © 2006 - Mathijs Lagerberg - http://mathijs.jurresip.nl", 5,
				(int) (this.dim.getHeight()) - 40);
	}
