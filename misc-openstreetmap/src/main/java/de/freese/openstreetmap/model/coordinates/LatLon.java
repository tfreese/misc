...
public class LatLon extends Coordinate
{
	 ...
	 */
	public static double course(final double lat1, final double lon1, final double lat2,
								final double lon2)
	{
		return Math.atan2(Math.sin(lon1 - lon2) * Math.cos(lat2), (Math.cos(lat1) * Math.sin(lat2))
	 ...
	 */
	public static double deriveCourse(final double lat1, final double lon1, final double lat2,
										final double lon2)
	{
	...
	/**
	 * From Aviaton Formulary v1.3 http://williams.best.vwh.net/avform.htm Calculate the
	 * great-circle-distance. (Distance between 2 points of a sphrere.)
	 * 
	 * @param lat1 postition1
	 * @param lon1 postition1
	 * @param lat2 postition2
	 ...
	 */
	public static double dist(final double lat1, final double lon1, final double lat2,
								final double lon2)
	{
	...
	/**
	 * Computes the distance between this lat/lon and another point on the earth. Uses spherical law
	 * of cosines formula, not Haversine. Earth is ellipsoid. see
	 ...
	 */
	public static int distanceInMeters(final double lat1, final double lon1, final double lat2,
										final double lon2)
	{
		final int factor =
				(int) ((6378 - (21 * Math.sin(Math.toRadians((lat1 + lat2) / 2)))) * 1000);
		// final int factor = 6371000;
		final double p1lat = Math.toRadians(lat1);
		final double p2lat = Math.toRadians(lat2);

		return (int) (Math.acos((Math.sin(p1lat) * Math.sin(p2lat))
				+ (Math.cos(p1lat) * Math.cos(p2lat) * Math.cos(Math.toRadians(lon2 - lon1)))) * factor);
	}
	...
	/**
	 * Given a squared distance as returned by {@link #distance(Coordinate)}, calculate the
	 * aproximate distance in kilometers.
	 * 
	...
	public static double distanceToKilomters(final double aDist, final Projection projection)
	{
		...
		return (Math.sqrt(aDist) * projection.scaleFactor() * Projection.EARTH_CIRCMFENCE_IN_METERS)
				/ kilo;
	}
	 ...
	 */
	public static double xtd(final double lat1, final double lon1, final double lat2,
								final double lon2, final double lat3, final double lon3)
	{
	...
	/**
	 * @return <code>true</code>, if the other point has almost the same lat/lon values, only differ
	 *         by no more than 1/Projection.MAX_SERVER_PRECISION.
