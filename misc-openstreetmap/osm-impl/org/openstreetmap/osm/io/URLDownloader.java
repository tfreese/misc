...
public class URLDownloader implements RunnableSource {
     ...
     *            http://www.openstreetmap.org/api/0.5/relation/1234/full).
     */
        ...
        try {
            ...
            try {
                ...
                if (myUrl.getPath().contains("/0.5/") && !myUrl.getPath().contains("/0.6/")) {
                    ...
                    migrate.setSink(mySink);
                    parser.parse(inputStream, new org.openstreetmap.osmosis.core.xml.v0_5.impl.OsmHandler(migrate, true));
                } else {
            ...
            if (myRetryCount < 3 && e.getMessage().indexOf("Server returned HTTP response code: 501 ") != -1) {
                myRetryCount++;
