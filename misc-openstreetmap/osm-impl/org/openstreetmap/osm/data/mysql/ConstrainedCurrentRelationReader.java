 ...
 */
public class ConstrainedCurrentRelationReader extends ConstrainedBaseReader<Relation> {

    /**
     * my logger for debug and error-output.
     */
    static final Logger LOG = Logger.getLogger(ConstrainedCurrentRelationReader.class.getName());
        ...
        private static final String SELECT_SQL =
            ...
            + " r.timestamp,"
            + " u.display_name"
            + " FROM current_relations r, users u {1}"
...
//        private int maxLon = Integer.MAX_VALUE;
         ...
         */
        public ConstrainedCurrentRelationReader(final DatabaseLoginCredentials aLoginCredentials) {
            super(aLoginCredentials);
            secondContext = new DatabaseContext(aLoginCredentials);
        }
...
//            FixedPrecisionCoordinateConvertor fixedPrecisionConvertor = new FixedPrecisionCoordinateConvertor();
//            additionalFrom = "";
//            additionalWhere = "";
//            this.minLat = fixedPrecisionConvertor.convertToFixed(Math.min(aminLat, amaxLat));
//            this.maxLat = fixedPrecisionConvertor.convertToFixed(Math.max(aminLat, amaxLat));
//            this.minLon = fixedPrecisionConvertor.convertToFixed(Math.min(aminLon, amaxLon));
//            this.maxLon = fixedPrecisionConvertor.convertToFixed(Math.max(aminLon, amaxLon));
//        }
         ...
         */
        public ConstrainedCurrentRelationReader(final DatabaseLoginCredentials aLoginCredentials, final long aRelationID) {
            this(aLoginCredentials);
...
//            this.maxLon = Integer.MAX_VALUE;
        ...
        public ConstrainedCurrentRelationReader(final DatabaseLoginCredentials aLoginCredentials,
            ...
            this(aLoginCredentials);
            this.secondContext = new DatabaseContext(aLoginCredentials);
...
//            }
        ...
        public Relation getNext() {
            ...
            try {
                ...
                String user = resultSet.getString("display_name");
            ...
            try {
                ...
                while (result.next()) {
                    ...
                    String memberTypeStr = result.getString("member_type");
                    String memberRole = result.getString("member_role");

                    EntityType memberType;
                    if (memberTypeStr.equalsIgnoreCase("way")) {
                        memberType = EntityType.Way;
                    } else if (memberTypeStr.equalsIgnoreCase("node")) {
                        memberType = EntityType.Node;
                    } else if (memberTypeStr.equalsIgnoreCase("relation")) {
            ...
            relation = new Relation(relationId, version, timestamp, null, tags, members);
            return relation;
        }
    };
