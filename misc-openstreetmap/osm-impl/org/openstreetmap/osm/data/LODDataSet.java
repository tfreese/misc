 ...
 *
 */
public class LODDataSet implements IDataSet {
...
// not all ways are known yet.
            ...
            if (myLOD1Selector.isAllowed(getLOD1DataSet(), aW)) {
              //simplify area
                ...
                WayBuilder simpleAreaWay = new WayBuilder(aW.getId(), aW.getVersion(), aW.getTimestamp(), aW.getUser());
...
//    }
    ...
    private Way extendedWayToWay(final Way aWay) {
        if (aWay instanceof ExtendedWay) {
            Way retval = new Way(aWay.getId(), aWay.getVersion(), aWay.getTimestamp(), aWay.getUser(), aWay.getTags(), aWay.getWayNodes());
            return retval;
        }
        return aWay;
    }
    ...
    private Node extendedNodeToNode(final Node aNx) {
        if (aNx instanceof ExtendedNode) {
            Node retval = new Node(aNx.getId(), aNx.getVersion(), aNx.getTimestamp(), aNx.getUser(), aNx.getTags(), aNx.getLatitude(), aNx.getLongitude());
            return retval;
        }
        return aNx;
    }
...
//            LOG.finer("DEBUG: new             as " + currentWayId + " way " + part.getId());
            ...
            WayBuilder currentWay1 = new WayBuilder(currentWayId, 0, new Date(), null);
            Map<String, Tag> tagsToPreserve = new HashMap<String, Tag>();
    ...
    private Relation getOrCreateCompoundWayLOD123(final Way aSimpleWay) {
        ...
        if (compound == null) {
            ...
            RelationBuilder compoundBuilder = new RelationBuilder((long) (Math.random() * Integer.MIN_VALUE), 0, new Date(), null);
